;(function(){
  ...
  var keyboard = {
    toggle: function() {
      var toggleBtn = document.querySelector('.create > .tool-btn')
    ...
    addEvent: function() {
      var aBtn = document.querySelectorAll('.keyboard > li > ul > li'),
          val = document.querySelector('.create-edit'),
          answer, valOri, string

      for( var i = 0, dio = aBtn.length; i < dio; i++) {

        if (aBtn[i].className === 'del') {
          aBtn[i].addEventListener('click',
          function() {
            valOri = val.value
            val.value = valOri.substring(0,valOri.length-1)
          })
        } else if (aBtn[i].className === 'equal') {
          aBtn[i].addEventListener('click',
          function() {
            if (val.value === '') {
              mio.text('<p><span>没数据，不给生成</span></p>')
            } else {
              answer = eval(val.value)
              create.saveDiv(answer)
            }
          })
        } else {
